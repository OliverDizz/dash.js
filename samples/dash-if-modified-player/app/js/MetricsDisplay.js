...
export class MetricsDisplay {
    // 1. Accept containerScope and suffix to isolate this instance
    constructor(playerController, chartController, containerScope, suffix) {
     ...
     */
    init() {
        // Bind chart toggle buttons (Scoped to this container)
        const toggleButtons = this.containerScope.querySelectorAll('.metric-chart-toggle');
        for (const btn of toggleButtons) {
            btn.addEventListener('click', () => {
                const metric = btn.dataset.metric;
                const type = btn.dataset.type;
                // Prefix the key so P1 and P2 metrics don't overwrite each other
                const key = `${this.suffix}-${type}-${metric}`;
                const label = `[${this.suffix.toUpperCase()}] ${type} ${metric}`;

                const isActive = btn.classList.toggle('active');
                if (this.chartController) {
                    this.chartController.toggleSeries(key, isActive, label);
                }
            });
        }
        ...
        for (const { metric, type } of defaultMetrics) {
            const btn = this.containerScope.querySelector(
            ...
            if (btn) {
                ...
                const label = `[${this.suffix.toUpperCase()}] ${type} ${metric}`;
                if (this.chartController) {
                    this.chartController.toggleSeries(key, true, label);
                }
            }
        }
    ...
    _onMetricsUpdate(data) {
        ...
        if (metrics.bufferLevel !== undefined) {
            ...
            const barEl = this._el(`${prefix}-buffer-bar`);
            if (barEl) {
        ...
        // Bitrate
        if (metrics.bitrate !== undefined) this._setText(`${prefix}-bitrate`, `${metrics.bitrate} kbps`);

        // Indices
        if (metrics.pendingIndex !== undefined) this._setText(`${prefix}-index-pending`, `${metrics.pendingIndex} / ${metrics.maxIndex}`);
        if (metrics.currentIndex !== undefined) this._setText(`${prefix}-index-current`, `${metrics.currentIndex} / ${metrics.maxIndex}`);

        // Dropped frames
        if (metrics.droppedFrames !== undefined) this._setText(`${prefix}-dropped`, String(metrics.droppedFrames));

        // HTTP metrics
        if (metrics.latencyAvg) this._setText(`${prefix}-latency`, `${metrics.latencyMin} | ${metrics.latencyAvg} | ${metrics.latencyMax}`);
        if (metrics.downloadAvg) this._setText(`${prefix}-download`, `${metrics.downloadMin} | ${metrics.downloadAvg} | ${metrics.downloadMax}`);
        if (metrics.ratioAvg) this._setText(`${prefix}-ratio`, `${metrics.ratioMin} | ${metrics.ratioAvg} | ${metrics.ratioMax}`);

        // CMSD metrics
        if (this._cmsdEnabled) {
            if (metrics.mtp) this._setText(`${prefix}-mtp`, (metrics.mtp / 1000).toFixed(1));
            if (metrics.etp) this._setText(`${prefix}-etp`, (metrics.etp / 1000).toFixed(1));
        }

        // Average throughput
        if (metrics.throughput !== undefined) this._setText(`${prefix}-throughput`, `${metrics.throughput} kbit/s`);

        // Video specific
        if (type === 'video') {
            if (metrics.resolution) this._setText(`${prefix}-resolution`, metrics.resolution);
            if (metrics.framerate) this._setText(`${prefix}-framerate`, `${metrics.framerate} fps`);
        }

        if (metrics.codec) this._setText(`${prefix}-codec`, metrics.codec);
        if (metrics.segmentDuration !== undefined) this._setText(`${prefix}-segment-duration`, `${parseFloat(metrics.segmentDuration).toFixed(2)} s`);
        if (metrics.bufferState) this._setText(`${prefix}-buffer-state`, metrics.bufferState === 'bufferLoaded' ? 'Loaded' : 'Stalled');
        if (metrics.playbackRate !== undefined) this._setText(`${prefix}-playback-rate`, `${parseFloat(metrics.playbackRate).toFixed(2)}x`);

        // Live-only metrics
        if (this._isDynamic) {
            if (metrics.liveLatency !== undefined) this._setText(`${prefix}-live-latency`, `${parseFloat(metrics.liveLatency).toFixed(2)} s`);
            if (metrics.targetDelay !== undefined) this._setText(`${prefix}-target-delay`, `${parseFloat(metrics.targetDelay).toFixed(2)} s`);
            if (metrics.dvrWindowSize !== undefined) this._setText('ms-dvr-window', `${Math.round(metrics.dvrWindowSize)} s`);
        }
    ...
    _plotMetrics(type, metrics, sessionTime) {
        ...
        for (const [metric, value] of Object.entries(keys)) {
            if (value !== null && value !== undefined && !isNaN(value)) {
                // Ensure data points are sent to the correct suffixed series
                chart.addDataPoint(`${this.suffix}-${type}-${metric}`, sessionTime, value);
            }
        }
    }
    ...
    _onSessionReset() {
        for (const prefix of ['mv', 'ma']) {
            ...
            const bar = this._el(`${prefix}-buffer-bar`);
    ...
    _toggleLiveMetrics(type, show) {
        const prefix = type === 'video' ? 'mv' : 'ma';
        const elements = [`${prefix}-live-section`, `${prefix}-live-latency-row`];

        if (type === 'video') elements.push(`${prefix}-target-delay-row`);
        ...
        if (type === 'video') {
            const dvrEl = this._el('ms-dvr-window-row');
            if (dvrEl) dvrEl.classList.toggle('d-none', !show);
        }
    }
    ...
    _setText(baseId, text) {
        const el = this._el(baseId);
